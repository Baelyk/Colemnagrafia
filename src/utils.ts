 ...
 */
export function resizeCanvas(gameOrCanvas: Game | HTMLCanvasElement, width: number, height: number): { scaling: number, width: number, height: number } {
 ...
 */
export function hexagon(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number) {
  const sides = 6;
 ...
 */
export function getTextHeight(ctx: CanvasRenderingContext2D, text: string): number {
  return ctx.measureText(text).fontBoundingBoxAscent + ctx.measureText(text).fontBoundingBoxDescent;
}

export function wrapText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, width: number): number {
  let height = 0;
  let line = "";
 ...
 */
export function shrinkFontSizeToFit(ctx: CanvasRenderingContext2D, text: string, width: number, startingFont: string): string {
 ...
 */
export function scrolling(game: Game, scroll: number, scrollSpeed: number, userIsScrolling: boolean, maximumScroll: number): [number, number] {
